...
class article{
	...
	function topic_del($pid,&$row){
		...
		$DB->query("delete from ".PF."article where id='$pid'");
	...
	function topic_move($pid,$tochannel_id){
		...
		if(!$row=$DB->fetch_first("select * from ".PF."article where id=".$pid))
			return;
		$channel_id=$row['channel_id'];
	...
	function channel_new($arr){
		...
		$pvs=$G['mods']['article']['pvs'];
		$configs=$G['mods']['article']['configs'];
	...
	function channel_del($id){
		...
		$rs=$DB->query('select * from '.PF.'article where channel_id='.$id);
	...
	function sign($sign,$pid=0){
		...
		if(file_exists(ABSPATH.'module/article/'.$sign.'.php')){
			return false;
		}
		if($DB->fetch_first("select id from ".PF."article where id<>'$pid' && sign='".addslashes($sign)."'")){
			return false;
		}
		return true;
	}
