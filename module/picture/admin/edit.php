...
$pid=(int)$_GET['pid'];
if(!$post=$DB->fetch_first('select *,picture.* from '.PF.'picture as picture left join '.PF.'user as user on user.id=picture.authorid where picture.id='.$pid)){
	yun_error('主题不存在');
}
...
if (IS_POST) {
	...
	if (strlen($sign)>0) {
		picture::sign ( $sign,$pid) or yun_error ( '非法的链接标志', 'yun_onfocus("sign");' );
	}
