...
class picture{
	function topic_new($pid,$row){
		...
		$str=sql_update($arr);
		$DB->query("update ".PF."picture set $str where id='$pid'");
	...
	function topic_del($pid,&$row){
		...
		$DB->query("delete from ".PF."picture where id='$pid'");
	...
	function topic_move($pid,$tochannel_id){
		...
		if(!$row=$DB->fetch_first("select * from ".PF."picture where id=".$pid))
			return;
		$channel_id=$row['channel_id'];
	...
	function channel_new($arr){
		...
		$pvs=$G['mods']['picture']['pvs'];
		$configs=$G['mods']['picture']['configs'];
	...
	function channel_del($id){
		...
		$rs=$DB->query('select * from '.PF.'picture where channel_id='.$id);
	...
	function sign($sign,$pid=0){
		...
		if(file_exists(ABSPATH.'module/picture/'.$sign.'.php')){
			return false;
		}
		if($DB->fetch_first("select id from ".PF."picture where id<>'$pid' && sign='".addslashes($sign)."'")){
			return false;
		}
		return true;
	}
