...
class tab_wenzi {
	function tpl($type, $id=0) {
		...
		ob_start ();
		if (is_file ( $file = ABSPATH . 'module/admin/tab_class/tpl/wenzi.php' ))
			include $file;
	...
	function tab_new($tabname) {
		...
		fint ( $conf [0], 0, 255 ) or yun_error ( '最少字数不在范围内', 'yun_onfocus("conf[0]");' );
		fint ( $conf [1], 1, 255 ) or yun_error ( '最多字数不在范围内', 'yun_onfocus("conf[1]");' );
		$conf [1] <= $conf [0] && yun_error ( '最多字数不得少于最少字数', 'yun_onfocus("conf[1]");' );
