...
(function($){
	...
	$.jNotify = {
		defaults: {
			...
			MinWidth : '25%',					// min-width CSS property
					...
					case 'top':
						...
						$Div.stop(true, true).delay(opts.TimeShown).animate({ // Tanks to Guillaume M.
							...
							if(opts.ShowOverlay && $("#jOverlay").length > 0)
								...
								if(!$("#fwin_xxx").length) $("#jOverlay").remove();
						});
						break;
					case 'center':
						if(!opts.autoHide)
	...
	jInfo = function(msg,options) {
		setTimeout(function(){$("#jLoading").fadeOut(300)},100);
		setTimeout(function(){$("#jLoading").remove();},400);
	...
	jDanger = function(msg,options) {
		setTimeout(function(){$("#jLoading").fadeOut(300)},100);
		setTimeout(function(){$("#jLoading").remove();},400);
		if($.jNotify._isReadable('jDanger')){
			$.jNotify.init(msg,options,'jDanger');
		}
	};
